...
#define HASH_NODENUM_MULTI		3
// hash version
static int* twoSum_V1(int* nums, int numsSize, int target) {
	int * ret = (int *)calloc(2, sizeof(int));
	...
	int hash_nodenum = numsSize * HASH_NODENUM_MULTI;
	t_hash_node * hash_table = (t_hash_node *)malloc( sizeof(t_hash_node) * hash_nodenum);

	int i;
	for(i = 0; i < hash_nodenum; i++){
		hash_table[i].index = -1;
		hash_table[i].next = NULL;
	}

	for(i = 0; i < numsSize; i++){
		int id = abs(nums[i]) % hash_nodenum;
		int id_peer = abs(target - nums[i]) % hash_nodenum;

		t_hash_node * node = &hash_table[id];
		t_hash_node * node_peer = &hash_table[id_peer];

		while(node_peer != NULL && node_peer->index != -1 && nums[node_peer->index] + nums[i] != target){
			node_peer = node_peer->next;
		}

		if(node_peer == NULL || node_peer->index == -1){
			if(node->index == -1){
				node->index = i;
			}else{
				t_hash_node * node_new = (t_hash_node *)malloc(sizeof(t_hash_node));
				node_new->index = i;
				node_new->next = node->next;
				node->next = node_new;
			}
		}else{
			ret[0] = node_peer->index;
			ret[1] = i;
			return ret;
		}
	}
...
{	
	...
	*/
	...
	int * nums = _input_array;
	int numsSize = sizeof(_input_array)/sizeof(int);

	gettimeofday(&tv0, NULL);
	//int * ret = twoSum_V0(nums, numsSize, 800);
	int * ret = twoSum_V1(nums, numsSize, 900);
	gettimeofday(&tv1, NULL);

	printf("get: %d-%d(%d-%d)\n", ret[0], ret[1], nums[ret[0]], nums[ret[1]]);
	printf("cost: %d s %d us\n", tv1.tv_sec - tv0.tv_sec, tv1.tv_usec - tv0.tv_usec);
