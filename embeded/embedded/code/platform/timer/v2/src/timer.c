...
static bool timer_dll_insert (dll_t *_p_dll, dll_node_t *_p_node, void *_p_inserted)
{
    timer_handle_t inserting = (timer_handle_t) _p_node;
    timer_handle_t inserted = (timer_handle_t) _p_inserted;

    if (inserted->round_ <= inserting->round_) {
        inserting->round_ -= inserted->round_;
        dll_insert_before (_p_dll, &inserting->node_, &inserted->node_);
        return false;
    }
    inserted->round_ -= inserting->round_;
    if (0 == dll_next (_p_dll, _p_node)) {
        dll_insert_after (_p_dll, &inserting->node_, &inserted->node_);
        return false;
    }
    return true;
}
...
static void timer_insert (timer_handle_t _handle)
{
    ...
    dll_remove (&g_inactive_timer, &_handle->node_);
    _handle->round_ = _handle->ticks_ / CONFIG_MAX_BUCKET;
    ...
    if (0 == dll_size (&g_buckets [_handle->bucket_index_].dll_)) {
        dll_push_tail (&g_buckets [_handle->bucket_index_].dll_, 
            &_handle->node_);
    }
    else {
        (void)dll_traverse (&g_buckets [_handle->bucket_index_].dll_, 
            timer_dll_insert, (void *)&_handle->node_);
    }
...
error_t timer_stop (timer_handle_t _handle)
{
    interrupt_level_t level;
    timer_handle_t next;
...
void timer_dump ()
{
    ...
    for (index = 0; index <= BUCKET_LAST_INDEX; ++ index) {
        console_print ("  [%u]: hit (%u), timer held (%u)\n", index, 
            g_buckets [index].hit_, dll_size (&g_buckets [index].dll_));
    }
    console_print ("\n");
    scheduler_unlock ();
}
