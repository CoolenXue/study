...
static void task_test (const char _name [], void *_p_arg)
{
    ...
    console_print ("============================\n");
    if ((result = heap_free (p_buf2, 32*1024)) != 0) {
    ...
    console_print ("=========================\n");
    if ((result = heap_free (p_buf1, 1)) != 0) {
        console_print ("heap_free () returns %s!\n", errstr (result));
    }
    console_print ("    Freed Addr: %p\n", p_buf1);
    heap_dump ();
    ...
    console_print ("============================\n");
    if ((result = heap_free (p_buf2, 64*1024)) != 0) {
        console_print ("heap_free () returns %s!\n", errstr (result));
    }
    console_print ("    Freed Addr: %p\n", p_buf2);
    heap_dump ();
    ...
    console_print ("============================\n");
    if ((result = heap_free (p_buf3, 64*1024)) != 0) {
        console_print ("heap_free () returns %s!\n", errstr (result));
    }
    console_print ("    Freed Addr: %p\n", p_buf3);
    heap_dump ();
